...
function postMash_getPages() {
    ...
    if ($pageposts == true) {
        ...
        foreach ($pageposts as $page): //list pages, [the 'li' ID must be pm_'page ID'] 
            ?>
            <li id="pm_<?php echo $page->ID; ?>"<?php if ($page->post_status == "draft") {
                echo ' class="remove"';
            } //if page is draft, add class remove ?>>
            ...
            <?php if ($switchDraftToPublishFeature): ?>
                            [<a href="#" title="Draft|Publish" class="excludeLink" onclick="toggleRemove(this);
                                    return false">toggle-draft</a>]
            <?php endif; ?>
                    </span>
...
function postMash_main() {
    ...
    <br style="margin-bottom: .8em;" />
        ...
        <?php // if (plugi('posts-export')) { ?>
        <div>
            <a style="margin-right: 40px; font-weight: bold; float:right;" href="<?php home_url(); ?>edit.php?page=posts-export\posts-export.php&yearno=<?= $_GET['yearno'] ?>&no=<?= $_GET['no'] ?>&tom=<?= $_GET['tom'] ?>">Выгрузить</a>
        </div>
       <?php //  } ?>
