...
function get_user_by($field, $value) {
	...
	switch ($field) {
		...
		case 'first_name':
			$value = sanitize_user( $value );
			$user_id = wp_cache_get($value, 'first_name');
			$field = 'first_name';
			break;
		default:
			return false;
...
if ( !function_exists('get_user_by_fio') ) :
/**
 * Retrieve user info by a given field
 *
 * @since 2.8.0
 ...
 * @return bool|object False on failure, User DB row object
 */
function get_user_by_fio($value) {
	global $wpdb;
	...
	if ( !$user = $wpdb->get_row( $wpdb->prepare("SELECT * FROM $wpdb->users left join $wpdb->usermeta on ($wpdb->users.id=$wpdb->usermeta.user_id)  WHERE meta_key='$field' and meta_value= %s", $value) ) )
		return false;
